...
function createMediaSender({ ilink, ffmpeg, ytdlp, mediaEnv }) {
  ...
  /** Build the media object for video_item. */
  function _videoMediaObj(up) { return _mediaObj(up); }

  /** Send a local video file as video_item. */
  async function sendLocalVideoFile(wxId, ctxToken, filePath) {
    if (!ctxToken) return;
    if (!fs.existsSync(filePath)) throw new Error(`视频文件不存在: ${filePath}`);
    const up = await ilink.uploadMedia(wxId, filePath, 2 /* VIDEO */);
    await ilink.sendItem(wxId, ctxToken, {
      type: 5,
      video_item: { media: _videoMediaObj(up), video_size: up.fileSizeCiphertext },
    });
    console.log(`[weixin-gateway] video sent to ${wxId} from ${filePath}`);
  }
  ...
  return { fetchToTempFile, sendImageFromUrl, sendLocalImageFile, sendLocalVideoFile, sendVideoFromUrl, downloadAndSendBilibili };
}
