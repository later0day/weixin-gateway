      ...
      const req = https.request({
        ...
        res.on('end', () => {
          ...
          try {
            ...
            // Top-level ret (e.g. {"ret":-2} on expired contextToken)
            if (parsed.ret !== undefined && parsed.ret !== 0) {
              return reject(new Error(`iLink API error type=${itemType} ret=${parsed.ret}${parsed.errmsg ? ' errmsg=' + parsed.errmsg : ''}`));
            }
            if (parsed.base_resp?.ret !== undefined && parsed.base_resp.ret !== 0) {
              return reject(new Error(`iLink API error type=${itemType} ret=${parsed.base_resp.ret} errmsg=${parsed.base_resp.errmsg}`));
            }
            if (parsed.errcode && parsed.errcode !== 0) {
              return reject(new Error(`iLink API error type=${itemType} errcode=${parsed.errcode} errmsg=${parsed.errmsg}`));
            }
          } catch {}
          resolve();
        });
